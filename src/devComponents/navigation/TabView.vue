...
<div :class="`flex ${borderTop ? 'flex-col-reverse' : 'flex-col'}`">
    <div class="header flex w-full border-gray-400" :class="`${borderTop ? 'border-t' : 'border-b'}`">
        <div class="relative cursor-pointer py-2 px-6 rounded-tr rounded-tl text-gray-700 hover:text-p-indigo-500" :class="{
          ' text-p-indigo-500': currentTab === index + 1,
            ...
            <transition :name="`slide-fade-${currentTab > prevTab ? 'right' : 'left'}`">
                <div v-if="currentTab === index + 1" :class="`${borderTop ? 'top-0' : 'bottom-0'}`" class="w-full absolute left-0 border-b-2 border-p-indigo-500 text-p-indigo-500"></div>
            </transition>
        </div>
    </div>
